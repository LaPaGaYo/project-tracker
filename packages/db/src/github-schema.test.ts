...
void test("github schema adds normalized engineering tables without replacing task_github_status", () => {
  ...
  assert.deepEqual(
    schemaTableNames.filter((tableName) =>
      githubTableNames.includes(tableName)
    ),
    [...githubTableNames]
  );
  ...
  assert.equal(
    getTableName(githubWebhookDeliveries),
    "github_webhook_deliveries"
  );
});
...
void test("github schema enums stay aligned with shared contracts", () => {
  assert.deepEqual(
    githubRepositoryProviderEnum.enumValues,
    githubRepositoryProviders
  );
  assert.deepEqual(
    githubPullRequestStateEnum.enumValues,
    githubPullRequestStates
  );
  assert.deepEqual(
    githubCheckRollupStatusEnum.enumValues,
    githubCheckRollupStatuses
  );
  assert.deepEqual(
    githubDeploymentEnvironmentEnum.enumValues,
    githubDeploymentEnvironments
  );
  assert.deepEqual(
    githubDeploymentStatusEnum.enumValues,
    githubDeploymentStatuses
  );
  ...
  assert.deepEqual(
    workItemGithubLinkSourceEnum.enumValues,
    workItemGithubLinkSources
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    githubWebhookDeliveryStatusEnum.enumValues,
    githubWebhookDeliveryStatuses
  );
});
...
void test("project GitHub connections persist conservative issue sync settings", () => {
  const columns = getTableConfig(projectGithubConnections).columns.map(
    (column) => column.name
  );
