...
test("buildGithubAppInstallUrl encodes workspace state", () => {
  ...
  assert.equal(url, "https://github.com/apps/the-platform-dev/installations/new?state=platform-ops");
});
...
test("normalizeGithubAppPrivateKey accepts escaped and base64 PEM values", () => {
  ...
  assert.equal(normalizeGithubAppPrivateKey({ privateKeyBase64: Buffer.from(privateKey).toString("base64") }), privateKey);
});
...
test("installation client mints an installation token and lists repositories", async () => {
  ...
  const client = createGithubAppInstallationClient({
    ...
    fetch: async (url, init = {}) => {
      ...
      if (serializeFetchUrl(url).endsWith("/app/installations/987/access_tokens")) {
  ...
  assert.equal(calls[1].url, "https://api.github.test/installation/repositories?per_page=100");
});
...
test("selected repository import rejects repositories outside the installation", async (t) => {
  ...
  await assert.rejects(
    ...
    (error) =>
      ...
      "status" in error &&
      error.status === 404 &&
      error.message === "selected repository is not available to this GitHub installation."
  );
});
